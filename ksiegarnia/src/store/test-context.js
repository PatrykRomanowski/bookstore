...
const testContext = createSlice({
    ...
    reducers: {
        addValue(state, actions) {
            state.value = new Decimal(state.value).plus(new Decimal(actions.payload.price)).toNumber();
        ...
        deleteItem(state, action) {
            // return state;
            const newListItem = state.items.filter((item) => item.id !== action.payload.id);
            state.items = newListItem;

            state.value = new Decimal(state.value).minus(new Decimal(action.payload.price).times(action.payload.counter)).toNumber();
